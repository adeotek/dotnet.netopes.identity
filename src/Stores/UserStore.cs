...
namespace Netopes.Identity.Stores
{
        ...
        where TRoleClaim : IdentityRoleClaim<TKey>, new()
    {
        ...
        /// </summary>
        public IUsersRecord<TUser, TKey, TUserClaim, TUserRole, TUserLogin, TUserToken> UsersRecord { get; }
        ...
        /// </summary>
        public IUserRolesRecord<TRole, TKey, TUserRole> UserRolesRecord { get; }
        ...
        /// </summary>
        public IRolesRecord<TRole, TKey, TRoleClaim> RolesRecord { get; }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (roleEntity == null) throw new InvalidOperationException($"Role '{normalizedRoleName}' was not found.");
        ...
        public override async Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken)
        {
            ...
            ThrowIfDisposed();
            user.ThrowIfNull(nameof(user));
            var userRoles = await UserRolesRecord.GetRolesAsync(user.Id);
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (role != null) users = (await UsersRecord.GetUsersInRoleAsync(normalizedRoleName)).ToList();
            return users;
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            ThrowIfDisposed();
            user.ThrowIfNull(nameof(user));
            if (string.IsNullOrEmpty(normalizedRoleName)) throw new ArgumentException(nameof(normalizedRoleName));
            var role = await FindRoleAsync(normalizedRoleName, cancellationToken);
            if (role != null)
            {
            ...
            string providerKey, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            var userLogin = await UserLoginsRecord.FindUserLoginAsync(userId, loginProvider, providerKey);
            return userLogin;
        }
        ...
        protected override Task<TRole> FindRoleAsync(string normalizedRoleName, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            var findRoleTask = RolesRecord.FindByNameAsync(normalizedRoleName);
            return findRoleTask;
        }
            ...
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            var userRole = await UserRolesRecord.FindUserRoleAsync(userId, roleId);
            return userRole;
        }
    }
}